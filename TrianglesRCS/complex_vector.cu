...
//}
//
__device__ complexFloat3 makeComplexFloat3(complex<float> a, complex<float> b, complex<float> c) {
	complexFloat3 res;
	res.x = a;
	res.y = b;
	res.z = c;
...
__device__ __forceinline void printComplexFloat3(complexFloat3 cf3) {
	printf("((%7f,%7f),(%7f,%7f),(%7f,%7f))\n", cf3.x.real(), cf3.x.imag(), cf3.y.real(), cf3.y.imag(), cf3.z.real(), cf3.z.imag());
}
...
inline __device__ void OrthonormalSet(float angP, float angT, float3& dirN, float3& dirU, float3& dirR)
{
...
inline __device__ void Orthonormalize(float3& dirN, float3& dirU, float3& dirR)
{
